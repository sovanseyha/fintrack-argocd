        ...
        script {
          catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
            withCredentials([usernamePassword(credentialsId: 'github_id', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')])  {
              ...
              sh "sed -i \"s+sovanseyha/ui-fintrack.*+sovanseyha/ui-fintrack:${DOCKERTAG}+g\" app/deployment.yaml"
              sh "git add ."
              sh "git commit -m 'Done by Jenkins Job changemanifest: ${env.BUILD_NUMBER}'"
              sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/${GIT_USERNAME}/fintrack-argocd.git HEAD:main"
            }
          }
        }
      }
